// Windows License Manager Service - probe only.
//
// Detects whether the LicenseManager service is running on Windows 10/11.
// Called from pre-launch diagnostics and the persistent-issues panel.
// The UAC-elevated start path has been removed; users can start the service
// manually via services.msc if it is stopped.

const { exec } = require("child_process");

async function checkWindowsLicenseManagerService() {
  return new Promise((resolve) => {
    exec("sc query LicenseManager", { timeout: 5000 }, (error, stdout) => {
      if (error) {
